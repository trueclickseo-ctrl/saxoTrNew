...
| `ATOS Saxo SIM Token Keepalive` | every 15 min, all day | `saxo_sim_token_keepalive.py` | **Added 2026-08-30.** Keeps the PKCE SIM refresh chain alive across the overnight gap. **Hardened 2026-09-01** (`82b2905`): `run_once()` now does a real `/users/me` call, not just time math — a bad `set_token.py` paste once left it reporting "OK" for hours against a 1-char token. In `WINDOWS_TASKS`. |
