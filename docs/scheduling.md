...
| `ATOS Saxo SIM Token Keepalive` | every 10 min, all day | `saxo_sim_token_keepalive.py` | **Added 2026-08-30.** Keeps the PKCE SIM refresh chain alive across the overnight gap. **Hardened 2026-09-01** (`82b2905`): `run_once()` now does a real `/users/me` call, not just time math — a bad `set_token.py` paste once left it reporting "OK" for hours against a 1-char token. **2026-09-03** (`79563b4`): task settings brought up to the LIVE keepalive's (`WakeToRun`/`StartWhenAvailable=True`, interval `PT15M`→`PT10M`); `_wrong_token_type()` now warns every tick + in the failure email when `saxo_token.json` is a **Developer-Portal 24h token** (`expires_in 86400`, no `refresh_token`) — the keepalive **cannot** roll one of those, so the real fix is a PKCE login (`python saxo_auth.py`, **not** `set_token.py`). In `WINDOWS_TASKS`. |
| `ATOS AI Health Email` | 09:00 + 21:00 PKT, daily | `run_ai_health_email.bat` → `ai_shadow_health.py --email` | **Added 2026-09-01** (`bcf5407`, `setup_ai_health_email.ps1`). A positive "AI shadow study is up and green" status email (or the problems, if any) — the watchdog only *alerts* on failure, this *confirms* health. Reads the two `data/ai_*.jsonl` logs + SMTP; no API key needed. In `WINDOWS_TASKS` (grace 20, first-run 13h). The same banner+metrics also render in the 23:30 daily summary. |
