...
### ⏭️ NEXT — Phase B (SIM flip) then Phase E (Limited LIVE)

**Immediate target: Phase B — Sprint 4 SIM flip (shadow_mode → false)**

Phase B gates on M5 (shadow evidence review, check 2026-09-13 via cloud routine reminder):
- Run `python report_phase_b_counterfactual.py sim` — **the key gate**: if net EUR effect is positive with ≥10 matched trades, that's the flip signal. Latest run 2026-09-08: **+415.3 EUR on 61 trades — POSITIVE and growing**. Re-run 2026-09-13 with more data.
- Run `python ai_shadow_report.py sim` — confirm clean APPROVE avg P&L > clean REJECT avg P&L with 10+ trades/bucket (strip XAU/XAG REJECT ≤2026-09-02, gold cost-bug artefacts)
- Run `python report_stop_slippage.py sim` — flag any strategy with mean slippage_r < −0.05 R
- Run `python report_exposure_controller.py sim --tighten` — tighten limits to Phase B targets (pair 2→1, cluster 8→6, risk 30→20%) before flip
- If all pass → flip `config/ai.json shadow_mode: false` → Sprint 4 (SIM) activates

**End goal: Phase E — Limited LIVE autonomy (coming weeks, forex first then stocks)**
- Phases B → C → D must ship and prove themselves on SIM first
- Phase C: autonomous exits (AI controls exit timing on SIM)
- Phase D: full SIM autonomy (AI manages full SIM book)
...
**Accumulating now:**
1. **Shadow evidence (M4/M5)** — target 10+ clean APPROVE/REJECT trades per bucket. Weekly: `python ai_shadow_report.py`. Next check: 2026-09-13 (cloud routine reminder).
2. **Phase B counterfactual** — latest run 2026-09-08: **+415.3 EUR on 61 trades** (POSITIVE, growing). Re-run 2026-09-13: `python report_phase_b_counterfactual.py sim`.
3. **Exposure controller data** — 1 block logged so far. After 1 week: `python report_exposure_controller.py sim --tighten`.
4. **Exit Copilot decisions** — starts populating `data/ai_exit_decisions.jsonl` from next exits-check cycle. Check after Phase B 3-week mark (~2026-10-04): `python report_phase_c_counterfactual.py sim`.
5. **Clean give-back data (P2)** — `python report_giveback.py`. When ≥10–15 clean trades/strategy, drives exit-improvement / SuperTrend-V2 decision.
