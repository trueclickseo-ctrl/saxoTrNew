...
| | |
|---|---|
| **AI live in production?** | **Forex SIM + Stocks SIM/IBKR paper ACTIVE** — `shadow_mode:false` for both. Entry Copilot applies REJECT/MODIFY to forex SIM (since 2026-09-11) and ALL stock strategies on Saxo SIM + IBKR paper (since 2026-09-26). Exit Copilot still shadow-only (`exit_copilot.shadow_mode:true`). AI Journal live (forex + stocks). |
| **AI touching money (SIM or LIVE)?** | **Yes — SIM/paper only.** Forex SIM: REJECT skips trades, MODIFY reduces qty. Stocks SIM (Saxo): same for reversion/signals/penny/bagger. IBKR paper: all 8 strategies gated (blend, blend_v2, reversion, reversion_v2, penny, bagger, signals, scorer). LIVE acting impossible without code change (`_AI_ACTING_ACCOUNTS = {"sim"}` — no live stocks entry). |
| **Last thing shipped** | **`(2026-09-26)`** — **Stocks AI Copilot flipped active + full IBKR wiring**: **(1)** `stocks.shadow_mode` flipped `false` (`d2f8bff`) — Copilot now applies REJECT/MODIFY to all Saxo SIM stock entries (reversion/signals/penny/bagger) and all 8 IBKR paper strategies. Same learning-from-live approach as forex Phase B. **(2)** All 4 `shadow_copilot_*` flags flipped `true` (`35c867e`) — Copilot now evaluates every Saxo SIM stock entry candidate and logs decisions. **(3)** IBKR observation cards wired for all 8 strategies (`3867faa`) + `_ai_ibkr_score()` gate fixed from `"sim"` → `"ibkr_paper"`. **(4)** `blend_v2` was the only IBKR strategy missing pre-trade copilot evaluation — wired (`2b02f1a`). **(5)** `check_ai_gates.py` added (`5adc9b1`) — weekly Monday 09:00 gate checker; sends status email always + "Copilot Ready" email when all three gates pass (B1, B2, C); scheduled via Claude App task. **(6)** `strategy` field added to stock exit cards (`38fd9b1`). **Phase B gate check 2026-09-26:** Gate 1 ✅ +1,117.3 EUR (95 trades), Gate 2 ❌ APPROVE avg −40.56 vs REJECT avg −14.24 (small-sample, n=10 APPROVE), Gate C ❌ 0 matched EXIT_NOW trades. Phase B stays active; next check 2026-10-06. **`(2026-09-22)`** — **Phase B gate check run + evolver schedule confirmed**: Gate 2 FAILED — APPROVE avg P&L −40.56 EUR vs REJECT avg −14.54 (agent approving losers). Other gates: counterfactual +1103.9 EUR on 90 trades ✅, stop slippage mean +0.72R ✅, exposure controller working ✅. **Phase B SIM flip stays active** (was flipped 2026-09-11); Gate 2 failing means quality is degraded — Copilot's APPROVE decisions are letting bad trades through. Re-run gates next Monday. **Evolver two-stage plan confirmed:** Run 1 when CNN-LSTM hits 30 forex_ai closed trades (~October 2026, currently 19 closed/5 open); Run 2 when ML hits 30 (~December 2026, currently 5 closed/17 open, ~0.36 closes/day). Run `python -m ai.agent.strategy_evolver --dry-run` every Monday for count tracking. **IBKR paper account confirmed DUR952126 on port 4002** (second Gateway, separate from live). IBKR paper scanner ran successfully after config switch. **`(2026-09-09)`** — **IBKR removed from AI pipeline + Unicode fixes in gate scripts**: (1) `ibkr_executor.py` — AI observation layer (entry/exit card hooks + `ibkr_stock_cards` import) removed (`9f5f3b8`). AI data pipeline is now Saxo-only; IBKR fills no longer feed `stock_observation_cards.jsonl`, Trade Journal, or Copilot. Reason: single consistent data source (same fill mechanics, currency, commission structure). (2) **Copilot promotion policy confirmed**: `agent_strategies` stays `["rsi","rsi_trend"]` until Phase B gate passes; add next 2 strategies only after Phase B data is sufficient — never bulk-add all strategies (cost blowup + noise). (3) **4 Phase B/C gate scripts fixed for Windows cp1252** (`6465a8e`): `report_phase_b_counterfactual.py`, `report_phase_c_counterfactual.py`, `report_exposure_controller.py`, `report_stop_slippage.py` — replaced `─`/`█`/`→`/`≥` with ASCII equivalents; all 4 Sep-13 gate scripts now run without `UnicodeEncodeError`. **`(2026-09-08, latest)`** — **Evolver second run + donchian_ai cooldown promoted + model tuning**: (1) `strategy_donchian_ai.py` — 12h per-symbol cooldown promoted from ai_variants override into the base strategy directly (1W/27L evidence: HKDJPY/JPYHKD/USDJPY stopped 16–19× each via rapid re-entry). COOLDOWN_HOURS=12.0, `_last_signal_ts` dict, timestamp recorded after each signal. (2) Evolver model finalized: copilot/journal/research/exit → Haiku; evolver → Sonnet 5 (weekly, ~$2–3/month). Haiku used for code writing was too risky for Phase 3 quality. (3) **Second evolver run (manual, 2026-09-08, `3d20b17`)** — 14 override files updated across all strategies: **Phase 2 new entry filters**: `rsi` blocks all JPY crosses (JPY hard_stop 2.4% WR vs non-JPY 69.6% — `rsi_recovery`), `donchian_ai` group cooldown on HKDJPY/JPYHKD/USDJPY correlated JPY basket. **Phase 3 exit updates**: `rsi` 2-bar stop confirmation; `donchian_ai` 2-bar stop (all 54 exits = hard_stop 29.6% WR); `ml` 2-bar stop (28.6% WR); `gap` 2-bar stop (1.7% WR / -4,077 EUR on hard_stop); `pullback` 2-bar EMA(50) trend_break preserved; `supertrend` 2-bar reversal confirmation (0% WR 19 losses). Pullback Phase 2 (DKKJPY filter) — Sonnet truncation ×2, deferred to next weekly run. Supertrend Phase 3 succeeded on retry. `--exit-only` flag confirmed working for targeted retry. **`(2026-09-08, earlier)`** — **AI model cost optimisation + evolver first real run**: copilot/journal/research_analyst/exit_copilot switched from `claude-sonnet-5` → `claude-haiku-4-5-20251001` (~20× cheaper per token, sufficient for APPROVE/MODIFY/REJECT verdicts). Evolver stays on Sonnet 5 (writes real strategy code). Evolver rescheduled weekly (Monday 01:30 PKT) — was daily, but strategies need 10–15 days to cross the 30-trade gate so daily runs wasted credits. **First successful evolver run** (2026-09-08, `eb25302`): 22 overrides written across 25 strategies — Phase 2 entry filters: `ema_trend` blocks DKK/HKD pegged pairs (97% of losses), `donchian_ai` 12h per-symbol cooldown (stops rapid re-entry whipsaw), `ml` blocks TRY+XAU, `gap` blocks exotic currencies (NOK/SEK/DKK/PLN/CZK/TRY/THB/HKD/MXN). Phase 3 exit overrides: `rsi`/`gap` 2-bar hard-stop confirmation (hard_stop was 1.7–2.4% WR), `pullback` 2-bar trend_break confirmation (6.7% WR), `supertrend` 2-bar reversal confirmation (0% WR 19 losses). All remaining strategies scaffolded with pass-through wrappers. Gate-ready: rsi(100), pullback(99), gap(100), ema_trend(61), donchian_ai(54), donchian(39), ml(34). Anthropic API key quota was exhausted ($20 cap hit); cap raised to $40. Estimated monthly AI cost now ~$3–5 (Haiku copilot ~$0.50 + weekly evolver ~$2–3). **`(2026-09-08)`** — **Full SIM strategy expansion + NZD contrarian + evolver scaled to 25 strategies**: (1) `SIM_ACTIVE_STRATEGIES` expanded 11→27 — all swing strategies now open SIM paper trades for AI data accumulation: re-activated retired strategies (donchian_quality, ml, supertrend), dormant originals (ema, bb, zscore, cnn_lstm), A/B experiments, gap/gap_weekend. `RETIRED_STRATEGIES` cleared. (2) `strategy_nzd_reversal.py` — AI-derived contrarian: reverses RSI(2) direction on NZD-containing pairs only. Data: NZD Buy loses across every strategy (RSI 27%WR/−380 EUR, Pullback 20%WR/−719 EUR, Gap 38%WR/−1827 EUR). Reversed signals expected ~73% WR. SIM-only research track. (3) `strategy_pullback.py` NZD filter + 2-bar trend_break confirmation wired in directly (applied to all SIM runs, not just ai_sim). (4) `ai/agent/strategy_evolver.py` expanded 4→25 forex strategies; 7 already gate-ready (≥30 closed trades): gap:245, rsi:152, pullback:99, ema_trend:61, donchian_ai:54, donchian:39, ml:34. Evolver rescheduled daily (01:30 PKT). **`(2026-09-08)`** — **Copilot JPY hard-REJECT rule** (`1664eb3`): upgraded JPY mean-reversion from MODIFY 0.5x to REJECT when `pair_history.win_rate_pct < 40%` (or 0% WR with n≥2). Driven by AI twin live track record: 0% WR across 8 JPY pairs (CNHJPY, GBPJPY, MXNJPY, NZDJPY, SGDJPY, CADJPY, EURJPY, JPYDKK) accounting for ~100% of RSI's −208 EUR loss in the AI twin. Cold JPY pairs (no history) now MODIFY 0.25x floor; APPROVE bar raised to n≥10 + WR>55% + clean RANGING + atr_ratio<1.0. Trend signals on JPY unaffected. **Phase B counterfactual updated** (232 decisions, 61 closed): net +415.3 EUR counterfactual gain (up from +264.6 EUR). MODIFY main driver: +478.6 EUR saved on losing trades at avg 0.48x. RSI leads by strategy (+388 EUR), RANGING leads by regime (+397 EUR). **AI twin closed trade count: 79** (gate: 30 passed — copilot now uses own `forex_ai` history, not SIM proxy). Reinforcement loop gate: 79/200. **`(2026-09-06, latest)`** — **Phase C — AI exit timing agent BUILT, ships shadow-only** (`ae48880`): `ai/agent/exit_copilot.py` evaluates open SIM positions that reached ≥1R MFE every exits-check cycle; actions = `HOLD \| EXIT_NOW` (TIGHTEN_STOP is v2); deduped once per position per day; logs every decision with `est_pnl_eur_at_decision` to `data/ai_exit_decisions.jsonl`; never raises. Runner hook added after the deterministic exit_advisor block (same guarded try/except pattern). `config/ai.json exit_copilot` block added (`enabled:true, shadow_mode:true, min_mfe_r:1.0`). `report_phase_c_counterfactual.py`: delta = est_pnl_at_EXIT_NOW − actual_exit_pnl (positive = give-back saved); gate = net positive + ≥60% good-call rate + ≥10 matched EXIT_NOW trades. Activation: flip `exit_copilot.shadow_mode: false` after Phase B 3-week evidence (~2026-10-04). **`(2026-09-06)`** — **Phase B counterfactual report built** (`05bb08c`): `report_phase_b_counterfactual.py` reads `ai_shadow_decisions.jsonl` + `pnl_ledger.db` and computes the net EUR effect if the Copilot had been applied. REJECT entered → delta = −realized_pnl (would have skipped); MODIFY entered at full size → delta = pnl × (multiplier − 1) (smaller size effect); APPROVE = baseline. **First live run (155 decisions, 55 matched):** net +264.6 EUR — baseline −554.9 → counterfactual −290.3. Main driver: MODIFY saved +335 EUR on losers (avg 0.48x). REJECT net −70 EUR (XAU bad blocks = gold cost-bug artefacts 2026-09-02, strip these). Verdict: POSITIVE — Phase B flip candidate at 2026-09-13 review. Run: `python report_phase_b_counterfactual.py sim`. **`(2026-09-06)`** — **Phase A — Portfolio Exposure Controller COMPLETE** (`78c97db`, `9d55b25`): `ai/portfolio/exposure_controller.py` — stateful-per-cycle book snapshot, gates new entry signals on 3 caps (pair_cap: max 2 same-symbol positions; cluster_cap: max 8 net-directional positions per currency; risk_cap: total open risk ≤30% of equity); runs BEFORE the Trading Copilot to avoid wasted LLM calls; logs every block to `data/exposure_controller.jsonl`; feeds `snapshot()` into the Copilot proposal context. Integrated into `forex/runner._run_entries()`. Config in `config/ai.json "exposure_controller"` block (defaults permissive for first week — tighten after 2026-09-13 gate). `report_exposure_controller.py` shows blocks by type, top blocked clusters/symbols, daily book risk % trend, false-positive candidates, `--tighten` suggests limit changes. First live data confirmed: 1 EURGBP pair_cap block on first SIM scan. GitHub MCP connector connected but cloud routine git-clone access not available (reminder routines stay reminder-only — data files are local). **`(2026-09-05, latest)`** — **AI Strategy Evolver Phase 2 FIRED — two AI-written overrides live on ai_sim** (`f2ce61b`): `forex/ai_variants/strategy_donchian_override.py` (filters exotic-quote pairs TRY/MXN/CZK/DKK/PLN/NOK/HUF/ZAR/SGD — AI finding: 20/30 trades, −577 EUR net, including CHFMXN stopped 4× in <2 hr); `forex/ai_variants/strategy_pullback_override.py` (filters NZD pairs — 14/35 trades = 71% of total −1101 EUR loss). Gate lowered 50→30 (legacy dedup artifacts leave ~35 quality pnl rows despite 74/117 total closed rows). `max_tokens` raised 2048→8192 for code generation. Both AST-validated, active `--account ai_sim` only, original `forex/strategy_*.py` untouched. Evolver re-runs weekly (Saturday 09:00 PKT). **`(2026-09-05)`** — **AI Strategy Evolver Phase 2 (forex code-writing) built + Copilot prompt hardened**: see log entry below. **`(2026-09-05)`** — **AI Strategy Evolver first dry-run**: both strategies returned confidence=low, no param changes (0 closed trades in `atos_ai.db` — US Reversion twin just started today). Claude correctly held at backtest-validated defaults rather than guessing. Re-run weekly; meaningful proposals expected once ≥15–20 closed trades accumulate per strategy. **`(2026-09-05)`** — **AI Strategy Evolver (Phase 1 — params only)**: `atos/ai_variants/__init__.py` (bounded param context managers), `ai/agent/strategy_evolver.py` (reads strategy source code + `atos_ai.db` trade history → calls `claude-sonnet-5` → proposes param overrides → writes `atos/ai_variants/us_blend_params.json` + `us_reversion_params.json` → logs to `data/ai_strategy_evolution.jsonl`). `atos_runner.py` `run_us_reversion` now accepts `account_env="ai_sim"` (paper-twin path, no Saxo orders). `atos_ai_stocks.py` now also runs US Reversion as a paper twin after US Blend. `ai_dashboard.py` shows US Reversion section (deterministic vs AI twin). Run manually: `python -m ai.agent.strategy_evolver` or `python -m ai.agent.strategy_evolver --dry-run`. **`(2026-09-05)`** — Saxo token keepalive refresh buffer raised 60s→300s (closes the 10-min dead window between the 15-min keepalive tick and the 20-min token TTL; was causing dashboard to show no live prices silently). **`(2026-09-05)`** — AI Research Analyst ROSTER expanded 5→9 strategies (`bb_quality_hv`, `zscore_quality_tb`, `donchian_ai` added to `decompose.py MODULE_IMPORT + ROSTER`; all 4 missing strategies now in the weekly decomposition sweep and auto-gate). **`(2026-09-05)`** — Multi-strategy agreement tracking: `pnl_tracker.get_concurrency_summary()` SQL self-join detects overlapping positions across strategies on the same pair; forex dashboard pair headers show historical WR when ≥2 strategies are live on that pair; Excel tracker gains Per-Pair × Strategy sheet (sheet 6) + Multi-Strategy Agreement sheet (sheet 7); both new digest keys (`by_pair_strategy`, `multi_strategy_agreement`) fed into AI research analyst. First live result: solo PF 0.72 vs 2-strategy agreement PF 3.09 vs 3-strategy PF 16.3 — strong early signal. **`(2026-09-05)`** — Saxo SIM stocks dashboard open positions now grouped by strategy; orphan IBKR stop cancellation script added. **`38602b7`** — IBKR universe 482→363. **`8cf8822`** — blend rebalancer bug fixed. **`01b87b3`** — IBKR WR/PF stats + IBKR PAPER section in ai_dashboard. |
...
### ⏭️ NEXT — Phase B/C quality watch + Stocks gate + Evolver

**Forex Phase B is ACTIVE** — flipped 2026-09-11. Copilot sizes SIM trades: APPROVE=full, MODIFY=reduced, REJECT=skip.
**Stocks Phase B is ACTIVE** — flipped 2026-09-26. Same mechanism on all Saxo SIM + IBKR paper stock strategies.

**Phase B quality gate check (2026-09-26 result — forex SIM):**
- Gate 1 — Counterfactual: ✅ +1,117.3 EUR on 95 trades
- Gate 2 — APPROVE > REJECT avg P&L: ❌ APPROVE avg −40.56 (n=10) vs REJECT avg −14.24 (n=34) — small-sample noise; one USDILS trade (−188 EUR) dominates. Will stabilize as n grows.
- Phase C — Exit matched trades: ❌ 0/10 matched EXIT_NOW closed trades yet

**Gate 2 note:** Copilot running on SIM. MODIFY is the main value-add (+582 EUR). APPROVE quality unstable due to tiny sample. Revert only if Gate 2 fails 3+ consecutive Mondays AND Gate 1 goes negative.

**Weekly gate checker (automated):** `check_ai_gates.py` runs every Monday 09:00 PKT via Claude App scheduled task. Sends status email every Monday + "Copilot Ready" email when all three gates pass simultaneously.

**Manual re-run:**
```bash
python check_ai_gates.py sim                    # all 3 gates + email
python report_phase_b_counterfactual.py sim    # Gate 1 detail
python ai_shadow_report.py sim                  # Gate 2 detail
python report_phase_c_counterfactual.py sim    # Gate C detail
```

**Phase C gate (~2026-10-06):** Gate: net positive delta + ≥60% good-call rate + ≥10 matched EXIT_NOW closed trades. Flip: `exit_copilot.shadow_mode: false` (user decision, Claude never flips).

**Stocks copilot quality gate (~2026-10-13):** Shadow data collection started 2026-09-26. At ~6–9 matched closes/week, gate clears ~2026-10-13. Run `check_ai_gates.py` — weekly email will alert automatically.

**Evolver two-stage plan (user decision 2026-09-22):**
| Run | Trigger | Strategies included |
|---|---|---|
| Run 1 (~Oct 2026) | CNN-LSTM hits 30 forex_ai closed trades (currently ~24 closed) | CNN-LSTM + RSI + DONCHIAN_AI + EMA Trend |
| Run 2 (~Dec 2026) | ML hits 30 forex_ai closed trades (currently ~5 closed) | ML specifically |
| Stocks evolver | 30 combined closed stock trades (~21/30, ~mid-Oct 2026) | US Reversion + US Blend |
...
- Phase E: limited LIVE (separate written go/no-go from user required; Claude never flips this)
- Estimated timeline: Forex ~2026-11-22, Stocks ~2026-12-01

**Accumulating now:**
1. **Forex Phase B quality** — Gate 2 failing (2026-09-26). Weekly email auto-checks. `python ai_shadow_report.py`.
2. **Forex Phase B counterfactual** — 2026-09-26: **+1,117.3 EUR on 95 trades** ✅. Weekly email tracks.
...
### Cost controls in place (`aace238`, updated `7b5e379`)
| Lever | Effect |
|---|---|
| `agent_strategies: ["rsi","rsi_trend","ema_trend","bb_quality","zscore_quality","donchian","pullback"]` | paid calls on 7 strategies only |
...
| Copilot/journal/research/exit → `claude-haiku-4-5-20251001` (`7b5e379`) | ~20× cheaper than Sonnet for verdicts |
| Evolver → `claude-sonnet-5` (weekly, Monday 01:30 PKT) | needs deep reasoning for code; weekly = ~$2–3/month |
| Anthropic console monthly spend cap | **$40** (raised 2026-09-08 from $20) |
| Est. total monthly spend | **~$3–5/month** (copilot ~$0.50 + evolver ~$2–3 + journal/research ~$0.50) |
...
- **Phase 1** (forex, `ai/research/decompose.py`): ROSTER = `[rsi, rsi_trend, rsi_atr, ema_trend, bb_quality, bb_quality_hv, zscore_quality, zscore_quality_tb, donchian_ai]` (9 strategies; expanded from 5 on 2026-09-05); 13y replay; cache `data/ai_research_decomp_cache.json`. Digest keys: `by_strategy`, `by_pair_strategy` (pairs with ≥2 strategies), `multi_strategy_agreement` (concurrency WR by solo/2/3/3+ agreed).
...
- **2026-09-26 — Stocks copilot fully wired + Phase B active for stocks**
  - `stocks.shadow_mode` flipped `false` (commit `d2f8bff`) — copilot now sizing SIM stock trades
  - All 4 `shadow_copilot_*` flags flipped `true` (commit `35c867e`) — Saxo SIM reversion/signals/penny/bagger scoring starts
  - IBKR `blend_v2` copilot wiring added (commit `2b02f1a`) — all 8 IBKR strategies now covered
...
- **2026-09-06 — Phase C exit timing agent built (shadow-only); Phase B counterfactual built; Phase A complete**
  ...
  **Phase C — Exit Copilot (`ae48880`):**
  - `ai/agent/exit_copilot.py`: LLM agent (claude-sonnet-5) evaluates open SIM positions ≥1R MFE each exits-check cycle. Actions: `HOLD | EXIT_NOW` (TIGHTEN_STOP is v2). Ships `shadow_mode:true` — logs every decision to `data/ai_exit_decisions.jsonl` with `est_pnl_eur_at_decision` (r_now × risk_eur) so the counterfactual can be computed. Deduped once per position per day. Never raises.
  - Runner hook: after the exit_advisor block in `_run_exits`; guarded try/except; only fires when position is in profit (`adv_result is not None`).
  - `config/ai.json exit_copilot`: `{enabled:true, shadow_mode:true, min_mfe_r:1.0, model:claude-sonnet-5}`.
  - `report_phase_c_counterfactual.py`: delta = est_pnl_at_EXIT_NOW − actual_exit_pnl. Gate: net positive + ≥60% good-call rate + ≥10 matched EXIT_NOW trades → flip `exit_copilot.shadow_mode:false`.
  - Activation timeline: data accumulates during Phase B SIM (~3 weeks); first meaningful check ~2026-10-04.

- **2026-09-06 — Phase B counterfactual report built; Phase A complete; Phase B + E roadmap confirmed**

  **Phase B counterfactual report (`report_phase_b_counterfactual.py`, `05bb08c`):**
  - Reads `ai_shadow_decisions.jsonl` + `pnl_ledger.db`; computes per-decision delta: REJECT entered = −pnl (saved loss / foregone gain); MODIFY full-size entry = pnl × (multiplier − 1).
  - First live run (155 decisions, 55 matched closed trades): net **+264.6 EUR** (baseline −554.9 → counterfactual −290.3). Verdict: **POSITIVE — Phase B flip candidate**.
  - MODIFY is the main driver: avg 0.48x multiplier wanted; +518 EUR saved on 28 losers, −183 EUR foregone on 18 winners = **+335 EUR net**.
  - REJECT net: −70 EUR — almost entirely 4 XAU pairs blocked 2026-09-02 (gold cost-bug artefacts; agent saw fake ~€3,700 cost and REJECTed valid winners). Strip those → REJECT near-flat.
  - By strategy: RSI +289.7 EUR, ema_trend +4.4 EUR, zscore_quality −29.4 EUR.
  - By regime: RANGING +166.6, TRENDING_BULLISH +93.7, TRENDING_BEARISH +4.4.
  - 46 decisions still pending (open trades). Re-run 2026-09-13 at M5 review.
  - Phase B checklist: counterfactual → shadow report → slippage → exposure tighten → flip.

- **2026-09-06 — Phase A complete; Phase B + E roadmap confirmed as next targets**

  **Phase A — Portfolio Exposure Controller (`78c97db`, `9d55b25`):**
  - `ai/portfolio/exposure_controller.py`: stateful book snapshot, 3 hard caps (pair_cap/cluster_cap/risk_cap), logs every block to `data/exposure_controller.jsonl`, feeds `snapshot()` into Copilot proposals. Integrated into `forex/runner._run_entries()`. Config defaults permissive (pair=2, cluster=8, risk=30%).
  - `report_exposure_controller.py`: blocks by type (bar chart), top blocked clusters, top blocked symbols with closed P&L cross-reference, daily book risk % trend, false-positive candidates, `--tighten` flag with data-driven suggestions.
  - First live confirmation: 1 EURGBP pair_cap block on first SIM scan after deploy.
  - **Limits to tighten after 2026-09-13 gate:** pair 2→1, cluster 8→6, risk 30%→20% (Phase B targets).

  **Roadmap confirmed:** Phase B (Sprint 4 SIM flip) → Phase C (autonomous exits) → Phase D (full SIM autonomy) → Phase E (limited LIVE). Forex first (~2026-11-22), Stocks after (~2026-12-01). Phase E requires separate written go/no-go from user. Claude never flips `shadow_mode` or `_AI_ACTING_ACCOUNTS` without explicit user instruction.

  **Cloud routines:** two reminder-only routines active (2026-09-12 evolver check, 2026-09-13 M5+slippage+exposure check). GitHub MCP connector connected but `sources.git_repository` cloud clone access not available — routines stay reminder-only (data files are local, cloud can't run the analysis scripts).

- **2026-09-05 — Token fix + AI ROSTER expansion + multi-strategy agreement analytics**

  **Saxo token refresh buffer (`saxo_auth.py`):**
  - `get_valid_access_token()` buffer raised from 60s to 300s. Math: keepalive task runs every 15 min; 20-min access_token − 5-min buffer = proactive refresh with 5 min to spare every tick. Old 60s buffer left a ~10-min dead window each cycle where the token had expired but the keepalive hadn't fired yet — forex dashboard showed no live prices silently with no alert.

  **AI Research Analyst ROSTER 5 → 9 (`ai/research/decompose.py`):**
  - Added `bb_quality_hv`, `zscore_quality_tb`, `donchian_ai` to `MODULE_IMPORT` and `ROSTER`. `rsi_atr` was also missing.
  - All 9 active SIM strategies now run through the weekly decomposition sweep + auto-gate. The 4 missing strategies had no journal trades yet so `by_strategy` digest still shows the 5 original keys until forward trades accumulate.

  **Multi-strategy agreement analytics (new tracking across 4 layers):**
  - `pnl_tracker.get_concurrency_summary(module)` — SQL self-join on `(symbol, strategy, timestamp_open/close)` detects overlapping closed positions across strategies on the same pair. Buckets: solo / 2 agreed / 3 agreed / 3+ agreed → per-bucket (trades, wins, WR%, total_pnl, profit_factor).
  - `forex_dashboard.py` — pair headers in the grouped open-positions view now show a `hist WR N% (Xt)` tag when ≥2 strategies are currently live on that pair and ≥2 historical closed trades exist. Colour-coded green/yellow/red by WR.
  - `reports/pair_group_performance_tracker.py` — two new sheets: **Per-Pair × Strategy** (sheet 6, one row per pair×strategy for pairs with ≥2 strategies, SUMIFS/COUNTIFS keyed on both axes from Trade Detail) and **Multi-Strategy Agreement** (sheet 7, concurrency WR table with plain-English explanation column).
  - `ai/features/research_analyst.py` — `build_research_digest()` now includes `by_pair_strategy` (per-pair×strategy WR slice from AI journal trades) and `multi_strategy_agreement` (full concurrency bucket data). System prompt updated to describe both keys and instruct the AI to test whether agreement count predicts profitability.
  - **First live data**: solo book PF 0.72 (losing), 2-strategy agreement PF 3.09, 3-strategy agreement PF 16.3 — preliminary but strong signal for hypothesis generation.
...
- **2026-09-05 (Copilot verdict analysis + prompt hardening + Phase 2 evolver)** — Full analysis of 237 shadow decisions (all `applied: False`). APPROVE: 85.7% WR on TRENDING trades, 0% WR on JPY mean-reversion (GBPJPY/JPYHKD/DKKJPY all hit hard stops), ~50% on RANGING. MODIFY: losses dominated by concentration (2+ same-currency positions). REJECT: AI correctly identified bad trades in 75% of cases cross-checked. HOLD: infra/network timeouts, not signal quality. Net verdict: AI copilot is **helping on TRENDING / hurting or neutral on JPY mean-reversion**; concentration errors are a known MODIFY→loss pattern now addressed. Three prompt changes committed to `ai/agent/trading_copilot.py`:
  1. **JPY caution rule**: on any JPY cross mean-reversion signal, prefer MODIFY (0.5x) unless regime is clean RANGING with ATR ratio < 1.0 AND pair_history ≥10 trades WR > 55%. Trend signals on JPY = normal rules.
  2. **Concentration REJECT threshold**: 2 same-currency/same-direction open positions → MODIFY (0.5x); 3+ → **REJECT**, no exceptions. Was previously MODIFY for any concentration. Based on live concentration losses dominating the MODIFY loss list.
  3. These rules are data-driven (2026-09 forward live track record cited explicitly in the prompt).
- **2026-09-05 (Donchian + Pullback research tracks re-activated)** — removed `"donchian"` and `"pullback"` from `RETIRED_STRATEGIES` in `forex/runner.py`, added to `SIM_ACTIVE_STRATEGIES` as research-only tracks. Added both to `config/ai.json` `agent_strategies` — the paid Copilot now observes every donchian/pullback signal. Goal: accumulate 50 closed SIM trades toward the Phase 2 evolver gate. **Never in `LIVE_ALLOWED_STRATEGIES`.** Current count: ~35/50 (donchian + pullback combined, as of 2026-09-05).
- **2026-09-05 (Stocks signals Copilot enabled)** — `config/ai.json` `stocks.shadow_copilot_signals` flipped `false → true`. The 4 US Signals strategies (SMA Crossover, RSI Reversal, Momentum, Ensemble) now score every entry through the shadow Copilot (log-only, no apply path). Adds to the existing `shadow_copilot_reversion` (US Reversion). All 5 stock strategies now generate shadow decisions visible in `ai_shadow_decisions.jsonl` and `ai_shadow_report.py`.
- **2026-09-05 (AI Strategy Evolver Phase 2 — forex code-writing, BUILT)** — extends the evolver from stocks param-tuning to writing Python override modules for forex strategies. Architecture:
  - **`forex/ai_variants/__init__.py`** (NEW) — AST-validated override loader. `get_override(strategy_name)` loads `forex/ai_variants/strategy_<name>_override.py` if it exists, runs a full AST security scan (whitelisted imports: `pandas/numpy/math/ta/collections/datetime/functools/itertools/statistics/typing/forex`; forbidden calls: `eval/exec/open/compile/__import__/subprocess/os/sys/socket/urllib/requests`; must define `generate_signals()`), returns the module or None on any failure.
  - **`forex/runner.py`** (MODIFIED) — when `ACCOUNT_ENV == "ai_sim"`, loads overrides from `forex/ai_variants/` before the strategy loop. `strat_mod = _forex_ai_overrides.get(strat_name) or STRATEGIES[strat_name]`. Falls back to original on any load error.
  - **`ai/agent/strategy_evolver.py`** (EXTENDED) — Phase 2 added alongside Phase 1: `evolve_forex_strategy(strategy, dry_run)` reads strategy source code + `pnl_ledger.db` trade history + `ai_shadow_decisions.jsonl` copilot shadow → calls `claude-sonnet-5` with `_FOREX_SYSTEM` prompt → receives `{"code": "..."}` JSON → AST-validates → writes `forex/ai_variants/strategy_<name>_override.py`. New helpers: `_fetch_forex_trade_history()`, `_fetch_copilot_shadow()`, `_ast_validate_override()`. New CLI flags: `--forex-only`, `--stocks-only`. `_call_claude` refactored to accept `system=` param (both phases share it). ThinkingBlock fix: `next((b.text for b in msg.content if hasattr(b, "text")), "")` throughout.
  - **Gate**: Originally 50; lowered to 30 (`_FOREX_CODE_GATE = 30`) — legacy dedup/reconcile artifacts (`reconciled_no_state`, `ledger_dedup_2026-09-01`) leave ~35 quality rows (non-null, non-zero `realized_pnl`) even though total closed rows are 74/117. Query filters `realized_pnl IS NOT NULL AND realized_pnl != 0`.
  - **Governance**: main `forex/strategy_*.py` is NEVER modified. Every override logged to `data/ai_strategy_evolution.jsonl`. LIVE accounts never use overrides. Weekly `run_ai_strategy_evolver.bat` (Saturday 09:00 PKT) covers Phase 2.
- **2026-09-05 (`f2ce61b`) — Phase 2 evolver FIRED — AI-written overrides now live on ai_sim**
  - `max_tokens` raised 2048→8192, timeout 90→180s (needed for full Python module in JSON).
  - **`forex/ai_variants/strategy_donchian_override.py`** (AI-written, confidence=medium): filters exotic-quote-currency pairs from Donchian signals. AI finding from 30 sampled closed trades: 20 trades on TRY/MXN/CZK/DKK/PLN/NOK/HUF/ZAR/SGD crosses, −577 EUR net, including CHFMXN stopped out 4× in under 2 hours. Majors and commodities (e.g. GC) were the source of essentially all winners. Filter logic: `_is_exotic_pair(symbol)` checks the symbol string for the 9 exotic currency codes; matching signals are dropped before they reach the runner. AST-validated ✅, passes `get_override("donchian")` ✅.
  - **`forex/ai_variants/strategy_pullback_override.py`** (AI-written, confidence=medium): filters NZD-cross pairs from Pullback signals. AI finding: 14 of 35 closed trades involved NZD (NZDHKD, NZDUSD, NZDSGD, NZDPLN, NZDCNH) and produced 71% of total losses (−782 of −1101 EUR), including two ~−210 EUR hard-stops on the same roster-entry timestamp. Filter: drop any signal whose symbol contains "NZD". AST-validated ✅, passes `get_override("pullback")` ✅.
  - Both overrides active for `--account ai_sim` only; original `forex/strategy_*.py` files untouched; ai_sim runner applies them from the next scan. Evolver re-runs weekly; next iteration refines or adds a second filter as more data accumulates.
- **2026-09-06 (`8d97d8e`) — Phase 3 exit-logic overrides FIRED for rsi + ema_trend**
  - Phase 3 extends the evolver to write `should_exit()` wrappers based on exit_reason breakdown. Pattern found across strategies: single-bar whipsaw on hard_stop / trend_break exits → fix = require 2 consecutive closes past the trigger.
  - **donchian + pullback** (previous session): combined Phase 2+3 overrides already written. Donchian 2-bar hard_stop confirmation (WR 27.8%, 13 losses/18 exits); Pullback 2-bar trend_break confirmation (WR 6.7%, 14/15 losses).
  - **`forex/ai_variants/strategy_rsi_override.py`** (NEW, confidence=medium): hard_stop had 2.7% WR (36/37 losses, −2,166 EUR combined). Wrapper blocks the exit if only a single daily close is past stop_price; requires 2 consecutive closes to confirm. Pass-through `generate_signals()` (no Phase 2 entry filter for RSI yet). AST-validated ✅, loads via `get_override("rsi")` ✅.
  - **`forex/ai_variants/strategy_ema_trend_override.py`** (NEW, confidence=low): 56/57 exits all `hard_stop` (23.2% WR), one `roster_flatten` — no second exit pattern to fix. Model correctly chose pass-through for both wrappers. AST-validated ✅, loads via `get_override("ema_trend")` ✅.
  - All 4 strategies now have overrides active on `ai_sim`. `rsi_trend`/`bb_quality`/`zscore_quality` skipped (fewer than 15 quality closed trades).
  - `__future__` added to AST whitelist in both the evolver's `_ast_validate_override()` and `forex/ai_variants/__init__.py`._ALLOWED_TOP_MODULES.
- **2026-09-06 (`2c9aa49`) — P4 stop-slippage measurement shipped**
  - **Why:** "strategy losses" on hard_stop may partly be execution losses (spread+latency at stop trigger time). P4 measures the gap between intended stop_price and actual fill_price.
  - **`forex/runner.py`** — `_SLIPPAGE_LOG = data/stop_slippage.jsonl`; `_log_stop_slippage(strategy, symbol, direction, stop_price, fill_price, entry_price, df)` called after every hard_stop close (non-dry-run, non-paper). Computes: `slippage_price` (fill − stop, signed), `slippage_atr` (÷ ATR14), `slippage_r` (÷ |entry−stop|). Negative = adverse.
  - **`report_stop_slippage.py`** (NEW) — reads the JSONL, aggregates by strategy: mean/median slippage in price, ATR×, R multiples, adverse%. `python report_stop_slippage.py [account]`. Interpretation: `mean slippage_r < −0.05 R` → execution losses eating measurable fraction of risk budget.
  - Log file grows as hard_stop exits accumulate. Run report in ~1–2 weeks once statistically meaningful.
- **2026-09-06 (`3a7fecc`) — Gold/silver proposal cost bug fixed + shadow report comments**
  - **Bug:** AI advisory block in `forex/runner.py` used `10_000` units for ALL pairs when computing all-in cost. Gold (XAU ~2,500 EUR/oz) → 10,000 oz = 25M EUR notional → ~€3,700 apparent cost vs €45 risk. Agent correctly REJECT-ed all XAU pairs as "cost-dominated" — but the input data was wrong, so those REJECT "wins" in M5 analysis were a data artefact, not agent skill.
