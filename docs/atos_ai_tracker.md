...
- Run `python ai_shadow_report.py sim` — need clean APPROVE avg P&L > clean REJECT avg P&L with 10+ trades/bucket (strip XAU/XAG REJECT trades ≤2026-09-02, gold cost-bug artefacts)
- Run `python report_stop_slippage.py sim` — flag any strategy with mean slippage_r < −0.05 R
- Run `python report_exposure_controller.py sim --tighten` — check if any cluster/pair/risk limits need tightening before allowing the AI to size trades
- If M5 passes → flip `config/ai.json shadow_mode: false` → Sprint 4 (SIM) activates
