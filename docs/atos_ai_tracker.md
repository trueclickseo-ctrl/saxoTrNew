...
### ⏭️ NEXT — Phase B quality watch + Phase C check + Evolver

**Phase B is ACTIVE** — flipped 2026-09-11. Copilot sizes SIM trades: APPROVE=full, MODIFY=reduced, REJECT=skip.

**Phase B quality gate check (2026-09-22 result):**
- Gate 1 — Counterfactual: ✅ +1103.9 EUR on 90 trades
- Gate 2 — APPROVE > REJECT avg P&L: ❌ **APPROVE avg −40.56 EUR vs REJECT avg −14.54** (agent approving losers — needs improvement)
- Gate 3 — Stop slippage: ✅ mean +0.72R, no strategy below −0.05R
- Gate 4 — Exposure controller: ✅ blocking correctly

**Gate 2 failing means:** Copilot is running on SIM but its APPROVE decisions are worse on average than REJECT. MODIFY is still the main value-add. Watch weekly; if APPROVE stays below REJECT for 3+ consecutive Mondays, investigate the system prompt or consider reverting `shadow_mode`.

**Weekly gate re-run (every Monday):**
```bash
python report_phase_b_counterfactual.py sim    # Gate 1
python ai_shadow_report.py sim                  # Gate 2
python report_stop_slippage.py sim              # Gate 3
python report_exposure_controller.py sim        # Gate 4
```

**Phase C gate check (~2026-10-04):**
- `python report_phase_c_counterfactual.py sim`
- Gate: net positive delta + ≥60% good-call rate + ≥10 matched EXIT_NOW closed trades
- Flip: `exit_copilot.shadow_mode: false` (user decision, Claude never flips)

**Evolver two-stage plan (user decision 2026-09-22):**
| Run | Trigger | Strategies included |
|---|---|---|
| Run 1 (~Oct 2026) | CNN-LSTM hits 30 forex_ai closed trades (currently 19 closed / 5 open) | CNN-LSTM + RSI + DONCHIAN_AI + EMA Trend |
| Run 2 (~Dec 2026) | ML hits 30 forex_ai closed trades (currently 5 closed / 17 open, ~0.36 closes/day) | ML specifically |

Track counts every Monday: `python -m ai.agent.strategy_evolver --dry-run`
Real run needs `ANTHROPIC_API_KEY` set in environment.
...
**Accumulating now:**
1. **Phase B quality** — Gate 2 failing (2026-09-22). Re-check weekly Monday: `python ai_shadow_report.py`.
2. **Phase B counterfactual** — 2026-09-22: **+1103.9 EUR on 90 trades** (POSITIVE). Re-run weekly.
3. **Phase C exit data** — `data/ai_exit_decisions.jsonl`. Gate check ~2026-10-04: `python report_phase_c_counterfactual.py sim`.
4. **Evolver forex_ai counts** — CNN-LSTM: 19, ML: 5 (17 open). Run dry-run weekly to track.
5. **Clean give-back data (P2)** — `python report_giveback.py`. When ≥10–15 clean trades/strategy.
