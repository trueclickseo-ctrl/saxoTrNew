...
| Sizing cap | `risk_equity_sek: 35000` (**15,000 → 35,000 on 2026-09-01**, Option A, after the 20,000 SEK deposit landed — sizes off the now-real ~35,800 SEK pool; `_risk_equity()` returns `min(real, cap)`) | `risk_equity_eur: 8000` (1,350 → 6,000 → **8,000**; **kept at 8,000 on 2026-09-01** — still ~2.4× the real ~€3,300 EUR share, a deliberate leverage choice). |
| RSI concurrency (8% heat cap) | ~5–6 concurrent (was ~2–3 at 15,000 SEK) — per-trade risk **unchanged** at `RSI_LIVE_FIXED_RISK_EUR = 45` (~500 SEK) | ~14 concurrent (8% of €8,000 / €45) |
