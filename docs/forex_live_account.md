...
**2026-09-01 — CONSOLIDATED to one account.** The Saxo sub-accounts have *separate* cash (`/balances/me` returns the sum): SEK ~25,700 SEK, EUR only ~€893, USD 0. The 20,000 SEK deposit landed in the **SEK** sub-account, so `live_eur` was running a ~€37k notional book on €893 of its own cash (the cause of its ±5% "no-trade" days). User moved all funds to the SEK account and runs RSI there only. `LIVE_EUR_ALLOWED_STRATEGIES = set()` (`c6af3c7`): the EUR account takes **no new entries** but its open positions stay exit-managed (`_legacy_exit_strategies(active=[], positions) → ['rsi'] → _run_exits`) until they close, then disable its scheduled tasks.

| Setting | SEK account (`--account live`) — **the live book** | EUR account (`--account live_eur`) — **entries off, exits only** |
|---|---|---|
| Strategy | `rsi` only (`LIVE_ALLOWED_STRATEGIES = {"rsi"}`) | `LIVE_EUR_ALLOWED_STRATEGIES = set()` — no new entries; open positions exit-managed until closed |
| Universe | 17-pair `HIGH_VOLUME_SYMBOLS` | (n/a — no new entries) |
| Scheduler | `ATOS Forex LIVE Daily Run` / `Exit Check` | `ATOS Forex LIVE EUR Daily Run` / `Exit Check` — leave running until EUR positions close, then disable |
...
| 20 | Closed-trade records could still drift from Saxo if the fill-confirmation fix (#17) ever mis-fires — nothing re-verified them | Low (backstop) | Added `reconcile_closed_trades_vs_saxo.py` (`7bd1f31`), run after every LIVE forex run: re-checks each recently-closed ledger row + observation card against Saxo's `ClosedPosition` (symbol+amount+side+close-time match) while it's in retention, corrects `entry_price`/`exit_price` drift >3 bp and re-scales `risk_eur`/`r_multiple`. Read-only w.r.t. trading, never raises, LIVE only (SIM `/closedpositions/me` → HTTP 400). `test_2026_09_01_reconcile_vs_saxo.py` (17). |
