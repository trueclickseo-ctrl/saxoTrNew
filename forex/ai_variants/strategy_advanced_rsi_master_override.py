...
def should_exit(position: dict, df: pd.DataFrame, calendar_days_held: int) -> Tuple[bool, str]:
    ...
    from the prior iteration: a hard_stop signal is only honored if the
    stop breach persists for two consecutive daily closes. A one-bar
    breach that closes back on the correct side of the stop is treated
    as noise and the position is held one more day (subject to the
    original logic re-evaluating on the next bar).
