...
def should_exit(position: dict, df: pd.DataFrame, calendar_days_held: int) -> tuple:
    ...
    band that reverse again before doing real damage. We require the
    ...
    trades respectively in this same dataset, both from unrelated causes
    like roster flattening).

    NOTE: this exit_reason breakdown (23 quality trades) is identical to
    the dataset the confirmation-bar rule was originally derived from --
    i.e. no new trades have closed under the updated logic yet, so there
    is no fresh evidence to refine the rule further. We therefore preserve
    the existing, already-justified confirmation-bar fix rather than
    layering an unproven second change on top of unvalidated data.
    """
