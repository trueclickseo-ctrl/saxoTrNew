...
def _economics(entry, stop, tp, commission_eur, risk_eur, all_in_cost_eur=None):
    try:
        ...
        # all-in transaction cost = commission + spread + slippage (the number
        # the deterministic LIVE recovery-vs-cost gate uses). Falls back to
        # commission alone when the caller could not estimate it.
        cost = all_in_cost_eur if all_in_cost_eur is not None else commission_eur
        if all_in_cost_eur is not None:
            out["all_in_cost_eur"] = round(all_in_cost_eur, 2)
        if risk_eur:
            tp_gross = risk_eur * rr
            out["risk_eur"] = risk_eur
            out["tp_gross_eur"] = round(tp_gross, 1)
            if cost is not None:
                out["tp_net_after_cost_eur"] = round(tp_gross - cost, 1)
                out["small_win_0p5R_net_eur"] = round(0.5 * risk_eur - cost, 1)
                out["breakeven_bounce_R"] = round(cost / risk_eur, 3)
                # the LIVE gate rejects when this ratio < 3.0 (a signal that
                # reached the agent already passed it -- near 3.0 = thin).
                out["recovery_0p5R_to_cost_ratio"] = round((0.5 * risk_eur) / cost, 2) if cost else None
        return out
    except Exception:
        return {"commission_eur": commission_eur}
