...
Testnet base URLs (hardcoded — no mainnet path exists here by design):
...
# The local binance/ package shadows the installed python-binance SDK.
# Temporarily remove the project root from sys.path so the SDK is found in site-packages.
_SDK_AVAILABLE = False
_BinanceSDK    = None
_bn_exc        = None

def _load_binance_sdk():
    """Import python-binance from site-packages, bypassing the local binance/ package."""
    import sys as _sys
    _proj_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    _removed = _proj_root in _sys.path
    if _removed:
        _sys.path.remove(_proj_root)
    try:
        from binance.client import Client         # type: ignore
        from binance import exceptions            # type: ignore
...
_BinanceSDK, _bn_exc = _load_binance_sdk()
_SDK_AVAILABLE = _BinanceSDK is not None
