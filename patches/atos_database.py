...
logger = get_logger("atos.database")
        ...
        cur = conn.execute("""
            ...
            VALUES
              (:market_group, :ticker, :direction, :entry_date, :entry_price, :shares,
               ...
               :trailing_stop_high, :regime_at_entry, :stop_price)
        """, data)
        logger.info("Trade inserted: %s %s (id=%d)", data.get('direction','?'), data.get('ticker','?'), cur.lastrowid)
...
def close_trade(trade_id: int, exit_price: float, exit_reason: str, pnl_sek: float,
    ...
    logger.info("Trade closed: id=%d reason=%s pnl=%.2f SEK", trade_id, exit_reason, pnl_sek)
        ...
        conn.execute("""
            ...
            VALUES
              (:signal_date, :market_group, :ticker, :final_score,
               ...
               :action, :executed, :block_reason)
        """, data)
    logger.debug("Signal logged: %s %s score=%.0f", data.get('action','?'), data.get('ticker','?'), data.get('final_score',0))
...
def save_weights(weights: dict, num_trades: int, note: str = ""):
    ...
    logger.info("Weights saved (from %d trades)", num_trades)
