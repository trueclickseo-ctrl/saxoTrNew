...
from atos.logger import get_logger, log_cycle_separator
logger = get_logger("run_atos")

# ── Pre-flight: ensure Saxo token is valid ────────────────────────
logger.info("Checking Saxo SIM token...")
try:
    import saxo_auth_auto
    if not saxo_auth_auto.refresh_existing():
        logger.warning("Token expired — launching automatic login...")
        saxo_auth_auto.login_auto()
    else:
        logger.info("Token is valid.")
except Exception as e:
    logger.warning("Could not verify token", exc_info=True)
    logger.warning("Cycle will attempt to proceed — may fail on Saxo API calls.")

# ── Monkey-patch: disable FTP upload ─────────────────────────────
import atos_runner
_original_upload = atos_runner.upload_dashboard
atos_runner.upload_dashboard = lambda local_file: logger.info(
    "[SKIP] FTP upload disabled — using localhost mode"
)

# ── Run the daily cycle ──────────────────────────────────────────
if __name__ == "__main__":
    log_cycle_separator(logger, "ATOS — Running in LOCALHOST mode (no FTP upload)")

    # Run the full trading cycle
    cycle_start = time.time()
    atos_runner.run_cycle()
    elapsed = time.time() - cycle_start
    logger.info("Total cycle time: %.1f seconds", elapsed)

    # Launch the local dashboard server
    logger.info("Starting local dashboard server...")
    try:
        import subprocess
        ...
        if os.path.exists(dashboard_script):
            ...
            logger.info("Dashboard launched at http://localhost:8070")
        else:
            logger.warning("atos_dashboard.py not found — run manually: py -3 atos_dashboard.py")
    except Exception as e:
        logger.warning("Could not auto-launch dashboard", exc_info=True)
